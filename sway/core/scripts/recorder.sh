...
if [ $status != 0 ]; then
    ...
    countdown
    (sleep 0.5 && $HOME/.config/sway/core/bin/waybar-signal recorder) &
    ...
    $HOME/.config/sway/core/bin/waybar-signal recorder && notify "Finished recording ${file}"
else
    pkill --signal SIGINT wf-recorder
    $HOME/.config/sway/core/bin/waybar-signal recorder
fi
